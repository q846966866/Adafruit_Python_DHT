...
def waitAndExecuteCommand(commandChannel):
    while True:
        ...
        # command can be a response of heart beat or an
        # update of the LED_control, so we split by ',' and
        # drop device id and device key and check length
...
def setLED(state):
    # Note the LED is "reversed" to the pin's GPIO status. So
    # we reverse it here.
    if state:
        GPIO.output(17,1)
    else:
